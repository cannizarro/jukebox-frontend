...
export default function playbackStateReducer(state: SpotifyStateType, action: ActionType): SpotifyStateType{
    ...
    switch(action.type){
        ...
        case PLAYBACK_PAUSE_FAILURE:
            return getStateForError(state, action.payload.response.status, false);
...
function getStateForError(state: SpotifyStateType, status: number, playingOnError: boolean): SpotifyStateType{
    switch(status){
        case 403:
            return {
                ...state,
